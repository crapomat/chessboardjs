 ...
 * Date: 10 Aug 2013
 *
 * Edited by devphp.de, added coordinates
 * This may be a copy of an older version, 
 * edit is quite some time ago, 
 * didn't find the time to release
 * search //This is edited by devphp.de
 */
...
//------------------------------------------------------------------------------
        ...
        function buildBoard(orientation) {
            ...
            // this is edited by devphp.de
            if (cfg.showNotationTopRight) {
                if (cfg.showNotationTopRight && cfg.showNotationLeftBottom) {
                    html += '<div class="coordz">&nbsp;</div>';
                }

                if (orientation !== 'black') {
                    for (var k = 0; k <= 7; k++) {
                        html += '<div class="coordx" style="width:' + SQUARE_SIZE + 'px;">';
                        if (cfg.showNotationLowerCase) {
                            html += COLUMNS[k] + '</div>';
                        } else {
                            html += letter[k] + '</div>';
                        }
            ...
            for (var i = 0; i < 8; i++) {
                ...
                for (var j = 0; j < 8; j++) {
                    ...
                    // this is edited by devphp.de
                    if (cfg.showNotationLeftBottom && j === 0) {
                        html += '<div class="coordybox" style="height:' + SQUARE_SIZE + 'px;"><div class="coordy">' + row + '</div></div>';
//    html += '<div class="coordy" style="height:' + SQUARE_SIZE + 'px;">' + row + '</div>';
                    }
                    ...
                    if (cfg.showNotationTopRight && j === 7) {
                        html += '<div class="coordybox" style="height:' + SQUARE_SIZE + 'px;"><div class="coordy">' + row + '</div></div>';
                    }
                }
                // this is edited by devphp.de
                if (cfg.showNotationLeftBottom && i === 7) {
                    html += '<div class="' + CSS.clearfix + '"></div></div>';
                    html += '<div class="coordz"';
                    if (cfg.moveIndicator) {
                        html += '  ' + CSS.moveIndicator + '"';
                    }
                    html += '>&nbsp;</div>';
                    if (orientation !== 'black') {
                        for (var k = 0; k <= 7; k++) {
...
// move pieces
        ...
        widget.resize = function () {
            ...
            // this is edited by devphp.de
            // make board width x more pixels, so coordinates fit in

            if (!cfg.showNotationLeftBottom && !cfg.showNotationTopRight) {
                boardEl.css('width', (SQUARE_SIZE * 8) + 'px');
            }
            if ((cfg.showNotationTopRight && !cfg.showNotationLeftBottom) || (!cfg.showNotationTopRight && cfg.showNotationLeftBottom)) {
                boardEl.css('width', ((SQUARE_SIZE * 8) + 15) + 'px');
            }
            if (cfg.showNotationLeftBottom && cfg.showNotationTopRight) {
                boardEl.css('width', ((SQUARE_SIZE * 8) + 30) + 'px');
            }
